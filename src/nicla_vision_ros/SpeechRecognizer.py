...
class SpeechRecognizer:
    ...
    def __init__(self, vosk_model_path: str, grammar: str, listen_seconds = 3, wave_output_filename = ""):
    ...
    def process_audio(self, data):
        ...
        if len(self.audio_buffer) > self.RATE*self.LISTEN_SECONDS:  # process in blocks of 1 second
            chunk = self.audio_buffer[:self.RATE*self.LISTEN_SECONDS]
            self.audio_buffer = self.audio_buffer[self.RATE*self.LISTEN_SECONDS:]
        ...
        if self.WAVE_OUTPUT_FILENAME: 
            if len(self.recording_frames) >= int(self.RATE / self.CHUNK * self.LISTEN_SECONDS):
