...
def draw_tf_on_image(cv_image:np.ndarray, cam_T_target:TransformStamped, cam_info:CameraInfo):
    ...
    target_R = quaternion_matrix(cam_T_target.transform.rotation)[:3, :3]
    ...
    if (0 <= p0[0] < cv_image.shape[1]) and (0 <= p0[1] < cv_image.shape[0]):
        ...
        rospy.logwarn("Origin out of bounds: %s", p0)
    
    cv2.line(cv_image, p0, px_end, (255, 0, 0), 4)  # Red X-axis
    cv2.line(cv_image, p0, py_end, (0, 255, 0), 4)  # Green Y-axis
    cv2.line(cv_image, p0, pz_end, (0, 0, 255), 4)  # Blue Z-axis
...
class TFOnImage:
    ...
    def run(self):
        ...
        for cam_T_target in self.cam_T_targets:
            draw_tf_on_image(cv_image, cam_T_target, self.cam_info)
